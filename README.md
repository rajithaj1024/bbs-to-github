...
- **Create two Environments:** Go to `Settings` → `Environments` → `New environment` and create both:
  - `bbs-migration` — used by the Pre-check, Validation and Monitoring stages. Do **not** add required reviewers here, or those stages will also wait for approval.
  - **Add Secrets:** Go to your GitHub repo → `Settings` → `Environment` → `ENV_NAME` → `Environment Secrets` → `Add environment secret`, and add the following:
  ...
  - **Add Variables:** Go to `Settings` → `Environment` → `ENV_NAME` → `Environment variables` → `Add environment variable`, and add:
  ...
  - **Add**`bbs-migration-approval` — used by the Migration stage only. Add one or more **Required reviewers** here; the migration job will pause and wait for approval before running.
...
- **Secrets/variables for the Migration stage** resolve against `bbs-migration-approval`. Repository-level secrets and variables (configured above) remain visible to both environments, so no duplication is needed unless you define environment-scoped overrides.
